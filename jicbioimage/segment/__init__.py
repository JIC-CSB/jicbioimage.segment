...
def connected_components(image, connectivity=2, background=None):
    ...
    :param connectivity: maximum number of orthagonal hops to consider a pixel/voxel as a neighbor
    :param background: consider all pixels with this value (int) as background
    """
    ar = skimage.measure.label(image, connectivity=connectivity, background=background)
    ...
    if background is None:
        ...
        # The jicbioimage.core.image.SegmentedImage assumes that the background
        # is labelled 0.
        ar[np.where(ar == 0)] = np.max(ar) +1
        ar[np.where(ar == -1)] = 0
